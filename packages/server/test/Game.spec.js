...
describe("Game().initGame() returns legalMoves", () => {
  it("initGame() returns default 19x19", (done) => {
    Game().initGame().legalMoves.should.eql(emptyBoard);
    done();
  });
...
describe("makeMove group join and basic capture logic", () => {
  ...
  it("stones in group have same group property", (done) => {
    joinGame.boardState["4-16"].group.should.eql(
      joinGame.boardState["5-16"].group
    );
    joinGame.boardState["4-16"].group.should.eql(
      joinGame.boardState["4-17"].group
    );
    joinGame.boardState["4-17"].group.should.eql(
      joinGame.boardState["4-16"].group
    );
    joinGame.boardState["4-17"].group.should.eql(
      joinGame.boardState["5-16"].group
    );
    joinGame.boardState["5-16"].group.should.eql(
      joinGame.boardState["4-17"].group
    );
    joinGame.boardState["5-16"].group.should.eql(
      joinGame.boardState["4-16"].group
  ...
  it("stones in group should have proper liberties", (done) => {
    ...
    joinGame.groups[group].liberties.size.should.eql(5);
    done();
  });
...
describe("capture logic: snapback, ko and playing in eyes", () => {
  ...
  it("snapback functions properly", (done) => {
    snapbackGame().success.should.eql(true);
    done();
  });
  ...
  it("ko recognized properly on Point", (done) => {
    koGame().boardState["5-5"].ko.should.eql(true);
    done();
  });
...
describe("Game history functionality", () => {
  ...
  it("Game.returnToMove returns new Game with new board state", (done) => {
    rewoundGame().boardState["16-4"].stone.should.eql(0);
    rewoundGame().boardState["4-4"].stone.should.eql(1);
    rewoundGame().boardState["16-16"].stone.should.eql(-1);
    done();
  });
  ...
  it("Game.returnToMove(5) returns to state after 5th move", (done) => {
    ...
    fifthMoveGame.gameRecord.should.eql([
      firstMove,
      secondMove,
      thirdMove,
      fourthMove,
...
describe("Game end logic", () => {
  ...
  it("pass changes game turn", (done) => {
    Game().initGame().submitPass("black").getMeta().turn.should.eql(-1);
    done();
  });
  ...
  it("end game counts territory properly", (done) => {
    const game = honinboGame
      .toggleTerritory("6-12")
      .toggleTerritory("6-16")
      .toggleTerritory("7-6")
      .toggleTerritory("7-6")
